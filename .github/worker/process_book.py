...
def existing_stem_signatures_for_book():
    signatures = set()
    try:
        result = (supabase.table("questions").select("stem").eq("book_id", BOOK_ID).limit(10000).execute())
        for row in result.data or []:
            stem = clean_text(row.get("stem") or "")
            if stem:
                signatures.add(re.sub(r"\W+", "", stem.lower()))
    except Exception as exc:
        print("[DB] Existing-stem lookup failed:", exc)
    print(f"[DB] Existing question signatures: {len(signatures)}")
    return signatures


def save_mcq_questions(questions, existing_signatures=None)
    if not questions:
        return 0
...
def process_mcq_book(pages, detected, existing_signatures=None)
    ...
    for i, batch in enumerate(batches, start=1):
        ...
        try:
            ...
            total += save_mcq_questions(prepared, existing_signatures)
        except Exception as exc:
...
def save_generated_questions(questions, existing_signatures=None)
    if not questions:
        return 0
...
def process_theory_book(chunks, existing_signatures=None)
    ...
    with ThreadPoolExecutor(max_workers=max(1, MAX_PARALLEL_AI)) as pool:
        ...
        for future in as_completed(future_map):
            ...
            try:
                ...
                saved = save_generated_questions(prepared, existing_signatures)
                total_saved += saved
...
def main():
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            existing_signatures = existing_stem_signatures_for_book()

            if is_mcq:
                total_questions = process_mcq_book(pages, detected_mcqs, existing_signatures)
            else:
                total_questions = process_theory_book(saved_chunks, existing_signatures)
